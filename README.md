...
- **选择页**：全部 429 名可获取干员，按稀有度 / 职业筛选、关键词搜索、一键预设（全选 / 仅六星 / 五星+六星 等）。
- **对比页**：每次展示两名干员，点击你更喜欢的一位；支持「势均力敌 / 都喜欢」（平局）、**撤销**、实时进度条。
  - 键盘快捷键：`←` 选左、`→` 选右、`↓` 或 `空格` 平局、`退格` 撤销。
- **结果页**：带并列名次的完整榜单；可复制榜单文本、生成分享链接、打印 / 存 PDF。
- 排序算法为**交互式归并排序**（用户充当比较器），对比次数约为 `n·log₂n`，并用并查集处理平局的等价名次。
